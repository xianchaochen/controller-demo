...
import (
	...
	clientset "github.com/chenxianchao/controller-demo/pkg/client/clientset/versioned"
	informers "github.com/chenxianchao/controller-demo/pkg/client/informers/externalversions"
)
...
func initClient() (*kubernetes.Clientset, *rest.Config, error) {
	...
	if home := homedir.HomeDir(); home != "" {
		fmt.Println(home)
		kubeconfig = flag.String("config", filepath.Join(home, ".kube", "kubeconfig"), "(可选) config 文件的绝对路径")
	} else {
		kubeconfig = flag.String("config", "", "config 文件的绝对路径")
	}
	flag.Parse()
